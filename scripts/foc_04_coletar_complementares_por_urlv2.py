...
def extrair_dados_basicos_contrato(page, numero_contrato):
    ...
    () => {
        ...
        const pegarProximoTexto = (label) => {
            const todos = Array.from(document.querySelectorAll('div, span, label, a, td, th')).filter(isVisible);
            ...
            for (const el of todos) {
                ...
                for (const c of candidatos) {
                    const t = normalizar(c.innerText || c.textContent || '');
                    if (!t) continue;
                    if (t === label) continue;
        ...
        const blocoTopo = document.body;

        return {
            numero_contrato_tela: pegarProximoTexto('CONTRATO'),
            cliente_nome: pegarProximoTexto('Cliente'),
            projeto: pegarProximoTexto('Projeto'),
            orcamento: pegarProximoTexto('Orçamento'),
            condicao_pagamento: pegarProximoTexto('Condição de Pagamento'),
            observacao: pegarProximoTexto('Observação'),
            observacao_pedido_compra: pegarProximoTexto('Observação do Pedido de Compra'),
            loja_unidade: pegarProximoTexto('Loja/Unidade'),
            tipo_venda: pegarProximoTexto('Tipo de Venda'),
            descritivo_contrato: pegarProximoTexto('Descritivo do Contrato'),
            liberacao_comercial: pegarProximoTexto('Liberação Comercial'),
            liberacao_financeira: pegarProximoTexto('Liberação Financeira'),
            andamento_obra: pegarProximoTexto('Andamento da Obra'),
            status_impressao: pegarProximoTexto('Status da Impressão'),
            data_aprovacao: pegarProximoTexto('Data de Aprovação'),
            assinatura_contrato: pegarProximoTexto('Assinatura do Contrato'),
            situacao: pegarProximoTexto('Situação'),
            link_meu_projeto: pegarProximoTexto('Link Meu Projeto')
        };
    }
    """
...
def extrair_dados_entrega_cobranca(page):
    ...
    () => {
        ...
        return {
            endereco_entrega: pegarProximoTexto('Endereço de Entrega'),
            endereco_cobranca: pegarProximoTexto('Endereço de Cobrança'),
            entrega_sugerido_dias: pegarProximoTexto('Entrega Sugerido (dias)'),
            prazo_entrega: pegarProximoTexto('Prazo de Entrega'),
            previsao_entrega: pegarProximoTexto('Previsão de Entrega'),
            observacoes_montagem: pegarProximoTexto('Observações de Montagem'),
            prioridade_entrega: pegarProximoTexto('Prioridade de Entrega')
        };
    }
    """
